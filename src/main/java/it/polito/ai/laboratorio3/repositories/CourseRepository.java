...
public interface CourseRepository extends JpaRepository<Course,String> {
    ...
    @Query("SELECT s FROM Student s INNER JOIN s.teams t INNER JOIN t.course c WHERE c.name=:courseName AND t.status=0")
    List<Student> getStudentsInInactiveTeams(String courseName);
